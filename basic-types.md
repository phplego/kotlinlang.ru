...
Например, функция с параметром `Double` может вызываться только для `Double`, но не для `Float`,
...
Если же нам необходима ссылка, которая может принимать значение null (например, `Int?`),
то используются обёртки. В этих случаях числа помещаются в классы Java как `Integer`, `Double` и так далее.
...
Они работают на двоичном уровне непосредственно с битовыми представлениям чисел.
...
или присвоить ей новое значение. Все операции, преобразующие строки, возвращают новый объект `String`,
...
Чтобы удалить пробелы в начале из обычных строк, используйте функцию
...
При необходимости вставить символ `$` в обычную строку (которая не поддерживает экранирование
