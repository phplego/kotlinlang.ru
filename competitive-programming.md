...
людей, имеющих опыт в спортивном программировании, но не использовавших ранее Kotlin. Также это руководство
...
Подробнее о том, как создать Kotlin-проект в IntelliJ IDEA, смотрите в руководстве
[Создание консольного приложения](https://kotlinlang.org/docs/jvm-get-started.html). В спортивном программировании
обычно создается один проект, а решение каждой задачи записывается в одном исходном файле.
...
задачи, которые мог бы решить любой разработчик. Вы можете воспользоваться
[этой ссылкой](https://codeforces.com/contest/1157), чтобы ознакомиться с ними. Самая простая задача в наборе -
[задача A: Достижимые числа](https://codeforces.com/contest/1157/problem/A). В ней предлагается реализовать простой
...
Обратите внимание на использование [оператора утверждения not-null](null-safety.html#not-null-assertion-operator) `!!`
после вызова функции [`readLine()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/read-line.html). Функция
Kotlin `readLine()` возвращает [nullable-тип](null-safety.html#nullable-types-and-non-nullable-types) `String?` и
...
[`println(...)`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/println.html) и
[строковые шаблоны](https://kotlinlang.org/docs/strings.html#string-templates) Kotlin. Однако следует быть осторожным,
когда вывод содержит порядка 10<sup>5</sup> строк или более. Выполнение такого количества вызовов `println` будет
...
[`joinToString()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/join-to-string.html) с `"\n"` в
...
Отличным ресурсом для изучения синтаксиса Kotlin и API стандартной библиотеки Kotlin являются
