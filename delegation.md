...
<!-- При переводе статьи оригинальная версия была от 29 May 2024 -->
...
Класс `Derived` может реализовать интерфейс `Base`, делегируя все свои публичные члены указанному объекту.
...
class Derived(b: Base) : Base by b {
    // Это свойство недоступно из реализации `print` в `b`
