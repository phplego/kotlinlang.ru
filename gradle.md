...
Данный плагин работает только для Kotlin-файлов, таким образом рекомендуется хранить Kotlin- и Java-файлы раздельно (это актуально, если проект содержит Java-файлы). Как и с поддержкой JVM, если не используются значения по умолчанию, необходимо определить папки ресурсов в *sourceSets*:
