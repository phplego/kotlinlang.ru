...
* Внутри [класса](classes.html#inheritance) `this` ссылается на текущий объект этого класса;
...
Если `this` не имеет определителей, оно ссылается на *самую внутреннюю охватывающую область*. Чтобы
сослаться на `this` в других областях, используются *метки-определители*.
...
помеченного [литерала функции с объектом-приёмником](lambdas.html#function-literals-with-receiver)), используйте
...
Когда вы вызываете функцию-член у `this`, часть `this.` можно опустить. Если существует функция, не являющаяся
членом, с тем же именем, используйте это с осторожностью: в некоторых случаях может быть вызвана именно она.
